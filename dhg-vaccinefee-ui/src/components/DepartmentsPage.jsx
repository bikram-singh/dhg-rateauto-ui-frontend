...
export default function DepartmentsPage({ departments = [], pricing = [] }) {
  ...
  return (
    <div>
      <div style={{ marginBottom:"16px" }}>
        <h2 style={{ color:"#fff", fontSize:"20px", fontWeight:"700" }}>Departments</h2>
        <p style={{ color:"rgba(255,255,255,0.6)", fontSize:"13px" }}>{departments.length} departments registered</p>
      </div>
        ...
        {stats.map((d) => (
          <div key={d.id} style={{
            ...
            <div style={{ display:"flex", alignItems:"center", gap:"12px", marginBottom:"14px" }}>
              ...
              <div>
                <div style={{ color:"#fff", fontWeight:"600", fontSize:"14px" }}>{d.name}</div>
                <div style={{ color:"rgba(255,255,255,0.5)", fontSize:"11px" }}>{d.description || "Medical department"}</div>
              </div>
            </div>
            <div style={{ display:"flex", justifyContent:"space-between" }}>
              <div style={{ textAlign:"center" }}>
                <div style={{ color:"#4FC3F7", fontSize:"20px", fontWeight:"700" }}>{d.count}</div>
                <div style={{ color:"rgba(255,255,255,0.5)", fontSize:"11px" }}>Pricing Records</div>
              </div>
              <div style={{ textAlign:"center" }}>
                <div style={{ color:"#4FC3F7", fontSize:"20px", fontWeight:"700" }}>₹{d.avgPrice}</div>
                <div style={{ color:"rgba(255,255,255,0.5)", fontSize:"11px" }}>Avg Price</div>
              </div>
            </div>
          </div>
        ))}
