...
export default function HospitalsPage({ hospitals = [], pricing = [], darkMode = true }) {
  ...
  return (
    <div>
      <div style={{ marginBottom:"16px" }}>
        <h2 style={{ color: t.text, fontSize:"20px", fontWeight:"700" }}>Hospitals</h2>
        <p style={{ color: t.textSec, fontSize:"13px" }}>{hospitals.length} hospitals registered worldwide</p>
      </div>
            ...
            <tr style={{ background: t.cardAlt, borderBottom:"1px solid rgba(255,255,255,0.1)" }}>
              {["#","Hospital Name","City","Location","Vaccines","Avg Price"].map((h) => (
                <th key={h} style={{ padding:"12px 16px", textAlign:"left", fontSize:"11px",
                  fontWeight:"600", color: t.textSec, textTransform:"uppercase", letterSpacing:"0.5px" }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {stats.map((h, i) => (
              <tr key={h.id} style={{ borderBottom:"1px solid rgba(255,255,255,0.05)" }}
                ...
                <td style={{ padding:"11px 16px", color: t.textMuted }}>{i+1}</td>
                <td style={{ padding:"11px 16px", color: t.text, fontWeight:"500" }}>{h.name}</td>
                <td style={{ padding:"11px 16px", color: t.text }}>{getCity(h.location)}</td>
                <td style={{ padding:"11px 16px", color: t.textSec, fontSize:"12px" }}>{h.location || "—"}</td>
