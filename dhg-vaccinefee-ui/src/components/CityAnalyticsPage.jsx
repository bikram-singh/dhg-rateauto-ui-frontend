...
export default function CityAnalyticsPage({ pricing = [], vaccines = [], hospitals = [], darkMode = true }) {
  ...
  return (
    <div>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:"20px", flexWrap:"wrap", gap:"10px" }}>
        <div>
          <h2 style={{ color: t.text, fontSize:"20px", fontWeight:"700" }}>City-wise Analytics</h2>
          <p style={{ color: t.textSec, fontSize:"13px" }}>Compare vaccine coverage across Indian cities</p>
        </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:`repeat(${Math.min(selectedStats.length,3)},1fr)`, gap:"12px", marginBottom:"20px" }}>
        {selectedStats.map((c, i) => (
          <div key={c.city} style={{ background: t.card, border:`1px solid ${COLORS[i]}40`,
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"8px" }}>
              {[
                ...
                <div key={s.label} style={{ background: t.cardAlt, borderRadius:"8px", padding:"8px 10px" }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"16px", marginBottom:"20px" }}>
        <div style={{ background: t.card, border:"1px solid rgba(255,255,255,0.1)", borderRadius:"12px", padding:"18px" }}>
          <div style={{ color: t.text, fontWeight:"600", fontSize:"14px", marginBottom:"14px" }}>Hospitals & Vaccines by City</div>
        ...
        <div style={{ background: t.card, border:"1px solid rgba(255,255,255,0.1)", borderRadius:"12px", padding:"18px" }}>
          <div style={{ color: t.text, fontWeight:"600", fontSize:"14px", marginBottom:"14px" }}>Coverage Radar</div>
          <ResponsiveContainer width="100%" height={220}>
            <RadarChart data={radarData}>
              ...
              <Legend wrapperStyle={{ fontSize:"11px", color: t.textSec }}/>
            </RadarChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div style={{ background: t.card, border:"1px solid rgba(255,255,255,0.1)", borderRadius:"12px", overflow:"hidden" }}>
        <div style={{ padding:"14px 18px", borderBottom:"1px solid rgba(255,255,255,0.08)" }}>
          <span style={{ color: t.text, fontWeight:"600" }}>All Cities — Complete Comparison</span>
        </div>
                ...
                <tr key={c.city} style={{ borderBottom:"1px solid rgba(255,255,255,0.05)" }}
                  ...
                  <td style={{ padding:"10px 14px" }}>
                    <div style={{ display:"flex", alignItems:"center", gap:"8px" }}>
                      ...
                      <span style={{ color: t.text, fontWeight:"500" }}>{c.city}</span>
                    </div>
                  </td>
                  ...
                  <td style={{ padding:"10px 14px", color: t.text }}>₹{c.avgPrice}</td>
                  <td style={{ padding:"10px 14px", color:"#4ADE80" }}>₹{c.minPrice}</td>
                  <td style={{ padding:"10px 14px", color: t.text }}>{c.available}</td>
